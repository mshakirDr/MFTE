...
data <- EvalData %>% 
  ...
  filter(!TagGold %in% c("NFP", "GW", "HYPH", "ADD", "AFX", "FW", "WQ", "SYM", "WDT", "MDother")) %>% 
...
cm$overall # Note that is not very meaningful because it includes tags which are not intended for use in MDA studies, e.g., LS and FW, or which are part of the evaluation process, e.g., NONE and UNCLEAR.
...
exclude_tags <- c("NONE", "UNCLEAR", "PRP", "NONE", "unclear")
data %>%
  select(Tag, TagGold) %>%
  filter(
    !(Tag %in% exclude_tags),
...
data = pandas.read_csv('/Users/Elen/Documents/PhD/Publications/2023_Shakir_LeFoll/MFTE_python/evaluation/MFTE_Python_Eval_Results_filtered.csv', keep_default_na=False)
